...
代码就变成了充满魔法数字 `(0, 1, -1)` 线性代数题，而不是业务逻辑的表达。

### 内存分配

为了解决易用性问题，社区出现了很多优秀的包装库 (例如 `now`, `carbon`)。它们提供了链式调用，读起来很舒服。但是我无法忍受它们的底层实现：每一次调用 (或是链式调用)，都在堆上分配内存！
...
我呆呆盯着屏幕上的 `GoMonth(1, 5, 3)` 方法许久。忽然间，仿如神灵附体，我似乎看见这三个参数 `1, 5, 3` 分别从屏幕中飞出来。**我知道了！**

> [!IMPORTANT]
>
> 如果我能调用 `GoMonth(1, 5, 3)`，那它和我链式调用 `GoMonth(1).GoDay(5).GoHour(3)` 有什么区别？
>
> 如果我能链式调用 `GoMonth(1).GoDay(5).GoHour(3)`，那它和我在一个循环中，通过 `applyAbs` 把每次调用的结果返回，并传给下一个循环的下一个时间单位 (如 Day)，又有什么区别！

一瞬间，我感觉整个时间维度坍缩了，它变得不再复杂和琐碎，我可以在每个原子操作中，将这一切都串联起来！也就是说，我可能需要在一个循环体中，陆续传入：
...
基于该原理，我分别实现了 4 个方法：`Go`, `By`, `Start`, `End`。但是我发现如果只有这些，还不够！

例如在当前导航系统中，我无法做到 **先偏移** 到某个时间点，再在此之上进行定位；反之，我也无法 **先定位** 到某个时间点，再在此之上进行偏移。
...
正是这 4 个完全正交的动作，构成了 Aeon 整个导航系统的核心，我认为它们几乎包含了所有可能的时间落脚点。该如何实现？
...
就这样，完了。我以为会很复杂...，结果解决方案之巧妙令我自己都感到惊讶，这就是 **正交** 的力量！

*`applyRel` 原理和 `applyAbs` 类似，它执行 **全相对偏移**，而非直接定位 (设置)。*

### 容器系统：时间索引

通过上述 4 个动作，我具备了几乎完整的时间导航系统，但是，我还想在此之上，更近一步。

我的目标是什么？我不想再将时间看作是线性的，例如 `AddDay().GoMonth().Start()`。这样每一次调用，都只是在上一个时间的基础上，做加减法而已，它本质仍然是线性运算，我想要的是 **一个具有统一层级的时间结构**。

---

当我们调用 `Go/ByMonth()`，很自然会觉得这就是在设置本年的月份，包括 `XXDay/Hour..()` 亦如此，在直觉上我们会认为，`Month` 就应该属于 `Year`，`Day` 属于 `Month`，`Hour` 又跑在 `Day` 里。

是的，这才是符合 **人类直觉** 时间观。它们是一个层级套着一个层级的，是一个容器，大单位包含着小单位，小单位又包含子单位。但是，`Year` 呢？`Year` 的上级又是什么？许多时间库到这里就结束了，它们没给年定义一个归属，年是孤立的。
...
虽然 Aeon 定位的核心是 “寻址”，但它还提供了 `Overflow` 标志，允许时间自然溢出。
...
> 这就是 Aeon 的故事。它不只是一个库，更是我对时间逻辑的一次重新构想。
>
> 如果你看到这里，忠实感谢你陪我走完这段心路历程，真心感谢！
