...
我定义了 `Start` 方法的原型为 `t.Start(u Unit, ...n)`。例如，如果我想获取某月份开始时间，就会调用 `Start(aeon.Month, 5)`，意图极其纯粹：定位到 5 月，然后将下级单位 (日、时、分、秒、纳秒) 全部抹平。
...
就这样，搞定？如果传 `0`，我会获取当前月份的开始时间，而不是设置新值。但是我怎么知道具体增加了几年几月呢？假如传入 `m=12` 超出一年怎么办？

为此，我设计了一个月份 ‘自动进位协议’。即便传入了不合常理的 `13` 或 `-1` 月，它都能像水流一样自动溢出到年份上，最终归位到正确刻度。
...
但是此时我还需要处理月份溢出问题，我应该怎么做？我写了一个 “获取每个月最大天数” 的方法：
...
这样我就能获取 `y` 年 `m` 月的最大天数了。其中 `maxDays` 被定义为：
...
但此时还有最后一个重要问题需要解决。那就是，我如何归零所有下级时间？
...
将所有这一系列方法串联起来，我得到了一个时间原子化定位方法：

*代码已经过简化，但原理是一样的，详细查看 [Aeon](https://github.com/baagod/aeon/blob/main/opus.go) 仓库的实现 (**不是推销！**)。*
...
此时，我终于拥有了一个绝对稳定的 **原子化时间操作引擎**，它就像是一把从未出鞘的神剑，一但出鞘必震动整个江湖！它能做什么呢？

## 维度的坍缩：级联架构

虽然我已经保证处理单一参数正确，但我的终极目标是 **级联参数**！通过传入变长参数，在这个原子方法中只返回一个指定的 `time.Date`，而不借助任何中间 `Time` 对象创建多个。

例如，我想要如下方法。无论级联多少参数，它只能创建一个 `Time` 对象并返回：

```go
GoMonth(1, 5, 3) // 设置时间为 1 月 5 日 3 点
```

我思索了很久，如果我继续尝试在 `applyAbs` 中，通过传入的这些所有变长参来改变每个 `case` 的实现，必然又会回到最初的令人恐惧那个 `if-else` 地狱。

我应该怎么办？现在我已经有了一个稳定单一时间操作方法。问题是，如何复用？该怎样复用？

我呆呆盯着屏幕上的 `GoMonth(1, 5, 3)` 方法许久。忽然间，我仿如神灵附体，似乎看到了这三个参数：`1`, `5`, `3` 分别从屏幕中飞了出来。我知道了！

> [!IMPORTANT]
> 
> 如果我能调用 `GoMonth(1, 5, 3)`，那它和我链式调用 `GoMonth(1).GoDay(5).GoHour(3)` 有什么区别？
> 
> 如果我能链式调用 `GoMonth(1).GoDay(5).GoHour(3)`，那它和我在一个循环中，通过 `applyAbs` 返回每次调用的结果，并传给下一个循环的下一个时间单位 (如 Day)，又有什么区别！

一瞬间，我感觉整个时间维度坍缩了，它变得不再那么复杂和琐碎，我可以在每个原子操作中，将这一切都串联起来！也就是说，我可能需要在一个循环体中，陆续传入：
...
这样，如果我将每个级联参数向下传递，将其结果传给下一个原子操作返回，最终我就能得到需要的时间！
(*为避免输入过程，方法已经过简化..*)
...
这一瞬间，我意识到 Aeon 的灵魂诞生了。整个过程，没有一次内存分配！全都是数字计算！你可以点击这里直接看这套引擎的实现：[Aeon](https://github.com/baagod/aeon)。

基于该原理，我实现了 Aeon 中最核心的 4 个正交方法：`Go`, `By`, `At`, `In`。

它们代表什么？我现在已经有了 `Start`, `End`, `Add (By)` 和 `Go` 系列方法，但是我发现如果只有这些，还，不，够！

例如在当前系统中，我无法做到先偏移到某个时间点，再在这个时间点上定位时间 (`Go`)。反之，我也做不到先定位到某个点，再在这个点上偏移时间 (`By`)。要做到这两点，我就还需要为 Aeon 再加入两个动作：

1. `At`: 首个参数绝对定位，后续参数偏移时间。例如：`At(5, 1)` -> 定位到 5 月再添加 1 天。
2. `In`: 首个参数偏移时间，后续参数绝对定位。例如：`In(1, 5)` -> 明年 5 月。

正是这四个完全正交的动作，构成了 Aeon 整个导航系统的核心。通过设计这 4 个动作，我认为它几乎包含了所有可能的时间落脚点。如何实现？
...
就这样，完了.. 我还以为会很复杂...，解决方案之巧妙令我都感到惊讶。

*注：`applyRel` 原理和 `applyAbs` 类似，只不过它使用 **全相对偏移**，而非直接定位 (设置)。*

## 容器系统：时间索引

终于，来到了这里，通过上面 4 个动作，我具备了几乎完整的时间导航系统，但是我还想在此之上，更近一步。

我的目标是什么？我不想再将时间看作是线性的，如 `AddDay().GoMonth().Start()`。这样每一次调用，都只是在上一个时间的基础上，做加减法而已，它本质上仍然是线性算术。我想要一个时间层级结构。

当我们调用 `Go/AddMonth()`，很自然会觉得这就是在设置本年的月份，包括 `XXDay/Hour..` 亦如此，在直觉上我们会认为，`Month` 就应该属于 `Year`，`Day` 属于 `Month`，`Hour` 又是在 `Day` 里跑的。

没错！就是这样，它们就是一个层级套着一个层级，是一个容器，大单位包含着小单位，小单位又包含子单位。但是 `Year` 呢？`Year` 的上级又是什么？许多时间库到这里就结束了，它们没给年单独定义一个层级，年是孤立的。

**但是，在 Aeon 中，我把这个概念彻底贯彻到了整个导航系统！**

我为每个动作，定义了一系列方法，从世纪到纳秒，例如这是 `Go` 系列：
...
只需选择你想要的时间入口，它就会自动生成该单位到纳秒的 **级联序列**。例如：
...
这就是 **时间容器**，当你进入一个入口单位，默认它就是在父级容器中进行索引，这样，你几乎可以去到任意时间点。并且还能反向索引，从父容器的末尾开始定位，例如：
...
这极大降低了使用者的心智负担：无需再为计算各种开始或结束边界感到困扰。
...
是的，但是为了不彻底将时间锁死在父层级里，我提供了 6 个顶级方法，允许传递的首个参数，去往 **真正的绝对年份**：
...
并且，虽然 Aeon 导航的核心是 “寻址”，但它还提供了 `Overflow` 标志，允许时间自然溢出。

```go
GoMonth(aeon.Overflow, 2) // 可能是 03-2/3
```

---

> 这就是 Aeon 的故事。它不仅是一个库，更是我对时间逻辑的一次重新理解。
>
> 如果你觉得这种 ‘容器模型’ 对你有启发，欢迎来 [Github](https://github.com/baagod/aeon) 踩踩。
> 
> 如果想知道更多细节，这里有一份详尽的 [仓库文档](https://zread.ai/baagod/aeon/1-overview)，请参考。

如果你看到这里，忠实感谢陪我走完这段心路历程，真心感谢！
